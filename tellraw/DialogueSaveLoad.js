        ...
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                ...
                MCFuncs.forEach((func) => {
                    let seqNum = parseInt(func.slice(125));
                    AddMCFuncLink(namePrefix + seqNum + "_trdg", func);
                });
...
function CreateNPC(dia) {
    let preComment = "# This code generated by Twined_Rope's Tellraw Generator (https://twinedrope.github.io/tellraw/TellrawGenerator.html)\n# Seq: " + dia.seqNum + "\n";
    return dia.tellraw[0] == '/' ? preComment + dia.tellraw.substr(1) + "\n" : preComment + dia.tellraw + "\n";
}
...
function CreatePlayer(dia) {
    ...
    if(dia.children[0]) {
        if(dia.children[0].link) {
            newTellraw = dia.tellraw.slice(dia.tellraw.indexOf("tellraw"),  splitIndex) + '"clickEvent":{"action":"run_command","value":"' + '/function generated_tellraw:' + namePrefix + '/' + namePrefix + dia.children[0].link + '_trdg"},' + dia.tellraw.slice(splitIndex);
        } else {
            newTellraw = dia.tellraw.slice(dia.tellraw.indexOf("tellraw"),  splitIndex) + '"clickEvent":{"action":"run_command","value":"' + '/function generated_tellraw:' + namePrefix + '/' + namePrefix + dia.children[0].seqNum + '_trdg"},' + dia.tellraw.slice(splitIndex);
        }
    } else {
